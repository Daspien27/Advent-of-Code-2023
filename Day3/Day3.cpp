...
std::vector<std::string> day3_input = {
...
int part1_solution(const auto& input) {
    auto scratch_space = day3_input;
    for (auto& row : scratch_space) {
        for (auto& col : row) {
            col = ' ';
        }
    }

    std::set<std::pair<int, int>> part_number_locations;

    for (int r = 0; r < day3_input.size(); ++r) {
        for (int c = 0; c < day3_input[r].size(); ++c) {
            ...
            bool valid_w1 = w1 >= 0;
            bool valid_w2 = w2 >= 0;
            bool valid_e1 = e1 < day3_input[r].size();
            bool valid_e2 = e2 < day3_input[r].size();
            bool valid_n1 = n1 >= 0;
            bool valid_s1 = s1 < day3_input.size();

            if (valid_n1 && valid_w1) adjacent_chars.push_back(day3_input[r - 1][c - 1]);
            if (valid_n1) adjacent_chars.push_back(day3_input[r - 1][c + 0]);
            if (valid_n1 && valid_e1) adjacent_chars.push_back(day3_input[r - 1][c + 1]);
            if (valid_w1) adjacent_chars.push_back(day3_input[r + 0][c - 1]);
            if (valid_e1) adjacent_chars.push_back(day3_input[r + 0][c + 1]);
            if (valid_s1 && valid_w1) adjacent_chars.push_back(day3_input[r + 1][c - 1]);
            if (valid_s1) adjacent_chars.push_back(day3_input[r + 1][c + 0]);
            if (valid_s1 && valid_e1) adjacent_chars.push_back(day3_input[r + 1][c + 1]);

            if (std::isdigit(day3_input[r][c])) {
                if (std::find_if(adjacent_chars.begin(), adjacent_chars.end(), [](char x) {
                    return !std::isdigit(x) && x != '.';
                    }) != adjacent_chars.end()) {

                    part_number_locations.emplace(r, c);

                    if (valid_w1 && std::isdigit(day3_input[r][c - 1])) {
                        part_number_locations.emplace(r, c - 1);
                        if (valid_w2 && std::isdigit(day3_input[r][c - 2])) {
                            part_number_locations.emplace(r, c - 2);
                        }
                    }

                    if (valid_e1 && std::isdigit(day3_input[r][c + 1])) {
                        part_number_locations.emplace(r, c + 1);
                        if (valid_e2 && std::isdigit(day3_input[r][c + 2])) {
                            part_number_locations.emplace(r, c + 2);
                        }
                    }
                }
    ...
    for (const auto& p : part_number_locations) {
        scratch_space[p.first][p.second] = day3_input[p.first][p.second];
    }


    int part_number_sum = std::transform_reduce(scratch_space.begin(), scratch_space.end(), 0, std::plus<int>{}, [](const auto& s) {
        std::regex re("\\d+");
        std::string remaining = s;
        int value = 0;
        for (std::smatch sm; std::regex_search(remaining, sm, re);)
        {
            value += std::stoi(sm.str());
            remaining = sm.suffix();
        }
        return value;
...
int part2_solution(const auto& input) {
    auto scratch_space = day3_input;
    for (auto& row : scratch_space) {
        for (auto& col : row) {
            col = ' ';
    ...
    std::vector<std::vector<int>> part_nos;
    part_nos.resize(scratch_space.size(), std::vector<int>((int)scratch_space[0].size(), 0));
    
    std::vector<int> col_vals;
    col_vals.reserve(3);
    for (int i = 0; i < scratch_space.size(); ++i) {

        const auto& part_no_agg = [&]() {
            if (col_vals.size()) {
                int part_no = std::accumulate(col_vals.rbegin(), col_vals.rend(), 0, [&, f=1 ](auto sum, auto v) mutable {
                    int r = sum + f * (scratch_space[i][v] - '0');
                        f *= 10;
                        return r;
                    });

                for (const auto& v : col_vals) {
                    part_nos[i][v] = part_no;
                }
        ...
        for (int j = 0; j < scratch_space[i].size(); ++j) {
            if (std::isdigit(scratch_space[i][j])) {
                col_vals.push_back(j);
            }
            else {
                part_no_agg();
            }
    ...
    for (int r = 0; r < scratch_space.size(); ++r) {
        for (int c = 0; c < scratch_space[r].size(); ++c) {
            if (scratch_space[r][c] == '*') {
                std::set<int> adjacent_parts;
                ...
                bool valid_w1 = w1 >= 0;
                bool valid_w2 = w2 >= 0;
                bool valid_e1 = e1 < scratch_space[r].size();
                bool valid_e2 = e2 < scratch_space[r].size();
                bool valid_n1 = n1 >= 0;
                bool valid_s1 = s1 < scratch_space.size();

                if (valid_n1 && valid_w1 && part_nos[r - 1][c - 1]) adjacent_parts.insert(part_nos[r - 1][c - 1]);
                if (valid_n1 && part_nos[r - 1][c + 0]) adjacent_parts.insert(part_nos[r - 1][c + 0]);
                if (valid_n1 && valid_e1 && part_nos[r - 1][c + 1]) adjacent_parts.insert(part_nos[r - 1][c + 1]);
                if (valid_w1 && part_nos[r + 0][c - 1]) adjacent_parts.insert(part_nos[r + 0][c - 1]);
                if (valid_e1 && part_nos[r + 0][c + 1]) adjacent_parts.insert(part_nos[r + 0][c + 1]);
                if (valid_s1 && valid_w1 && part_nos[r + 1][c - 1]) adjacent_parts.insert(part_nos[r + 1][c - 1]);
                if (valid_s1 && part_nos[r + 1][c + 0]) adjacent_parts.insert(part_nos[r + 1][c + 0]);
                if (valid_s1 && valid_e1 && part_nos[r + 1][c + 1]) adjacent_parts.insert(part_nos[r + 1][c + 1]);

                if (adjacent_parts.size() == 2) {

                    int gear_ratio = std::accumulate(adjacent_parts.begin(), adjacent_parts.end(), 1, std::multiplies<int>{});
...
int main()
{
    ...
    int part_number_sum = part1_solution(day3_input);

    std::cout << "Day 3 part 1 sum of part numbers: " << part_number_sum << std::endl;

    int gear_ratio_sum = part2_solution(day3_input);

    std::cout << "Day 3 part 1 sum of gear ratios: " << gear_ratio_sum << std::endl;
}
